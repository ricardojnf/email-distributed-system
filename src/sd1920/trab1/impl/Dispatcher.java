...
public class Dispatcher {
    ...
    private MessageProxy resourceProxy;
    private MessageReplicated resourceReplicated;
    ...
    public Dispatcher(String targetDomain, MessageResource resource) {
        ...
        this.resourceReplicated = null; //NOT SURE
    ...
    public Dispatcher(String targetDomain, MessageProxy resourceProxy){
        this.targetDomain = targetDomain;
        this.resource = null;
    ...
    public Dispatcher(String targetDomain, MessageReplicated resourceReplicated){
        this.targetDomain = targetDomain;
        this.resource = null;
        this.resourceProxy = null;
        this.resourceReplicated = resourceReplicated; //NOT SURE
        this.jobs = new LinkedBlockingQueue<>();
        this.signalStop = false;
    ...
    private void dispatchLoop() {
        ...
        while (!signalStop) {
            ...
            while (!done) {
                ...
                String secret;
                if(resource != null)
                    secret = MessageResource.secret;
                else if(resourceProxy != null)
                    secret = MessageProxy.secret;
                else
                ...
                if (response.getStatusCode() == Response.Status.SERVICE_UNAVAILABLE.getStatusCode()) {
                    ...
                    if (nextJob instanceof Job.DeliverJob) {
                        ...
                        else if(this.resourceReplicated != null){
                            resourceReplicated.createErrorMessage(dJob.getMessage().getSender(), dJob.getSenderName(),
                                    dJob.getMessage().getId(), dJob.getTarget() + "@" + targetDomain);
                            System.out.println("Error email sent to inbox -> " + dJob.getMessage().getId());
                        }
                        else{
                            resourceProxy.createErrorMessage(dJob.getMessage().getSender(), dJob.getSenderName(),
                                    dJob.getMessage().getId(), dJob.getTarget() + "@" + targetDomain);
                        }
    ...
    private MessagesEmailClient doDiscovery() {
        ...
        return ClientFactory.getMessagesClient(uris[0], 5, 20000);
    }
